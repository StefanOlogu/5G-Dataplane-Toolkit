...
pub fn handle_packet_header(
    bytes: &[u8],
    big_endian: bool,
    is_nano: bool,
    packet_count: usize,
) -> Result<PcapPacketHeader, Box<dyn Error>> {
    let unit = if is_nano {
        "nanoseconds"
    } else {
        "microseconds"
...
pub fn handle_ethernet_header(bytes: &[u8]) -> Result<EthernetHeader, Box<dyn Error>> {
    ...
    println!(
        ...
        ethernet::format_mac(ethernet_header.dest_mac())
    );
    println!(
        "  Source MAC address : {}",
        ethernet::format_mac(ethernet_header.src_mac())
    );
    println!("  EtherType: 0x{:04X}", ethernet_header.ether_type());
    Ok(ethernet_header)
}

pub fn handle_ip_header<'a>(
    ip_payload: &'a [u8],
    ether_type: u16,
) -> Result<TransportInfo<'a>, Box<dyn Error>> {
    match ether_type {
        0x0800 => {
...
pub fn handle_transport(payload: &[u8], protocol: u8) {
    match protocol {
        6 => match transport::parse_tcp_header(payload) {
            Ok(tcp_header) => {
                println!("  TCP Header:");
        ...
        17 => match transport::parse_udp_header(payload) {
            Ok(udp_header) => {
                println!("  UDP Header:");
