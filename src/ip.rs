...
pub fn parse_ipv4_header(bytes: &[u8]) -> Result<Ipv4Header, Box<dyn Error>> {
    ...
    let total_length = u16::from_be_bytes(
        bytes[2..4]
            .try_into()
            .map_err(|_| "Failed to parse total length")?,
    );
    let identification = u16::from_be_bytes(
        bytes[4..6]
            .try_into()
            .map_err(|_| "Failed to parse identification")?,
    );

    let flags_frag = u16::from_be_bytes(
        bytes[6..8]
            .try_into()
            .map_err(|_| "Failed to parse flags fragment")?,
    );
    ...
    let header_checksum = u16::from_be_bytes(
        bytes[10..12]
            .try_into()
            .map_err(|_| "Failed to parse header checksum")?,
    );
...
pub fn parse_ipv6_header(bytes: &[u8]) -> Result<Ipv6Header, Box<dyn Error>> {
    ...
    let payload_length = u16::from_be_bytes(
        bytes[4..6]
            .try_into()
            .map_err(|_| "Failed to parse payload_length")?,
    );
